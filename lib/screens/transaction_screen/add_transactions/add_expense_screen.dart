...
class AddExpenseScreen extends StatefulWidget {
  const AddExpenseScreen({super.key, this.modelFromTransaction});

  @override
  State<AddExpenseScreen> createState() => _AddExpenseScreenState();
  final TransactionModel? modelFromTransaction;
}
...
class _AddExpenseScreenState extends State<AddExpenseScreen> {
  ...
  @override
  void initState() {
    if (widget.modelFromTransaction != null) {
      String categoryFromTransaction =
          widget.modelFromTransaction!.category.id.toString();
      String accountTypeFromTransaction = '';
      if (widget.modelFromTransaction!.account == AccountType.account) {
        accountTypeFromTransaction = 'account';
      } else if (widget.modelFromTransaction!.account == AccountType.cash) {
        accountTypeFromTransaction = 'cash';
      }
      selectedDate = DateTime.parse(widget.modelFromTransaction!.date);
      _amountController.text = widget.modelFromTransaction!.amount.toString();
      accountType = accountTypeFromTransaction;
      _categoryID = categoryFromTransaction;
      selectedCategoryModel = widget.modelFromTransaction!.category;
      _noteController.text = widget.modelFromTransaction!.note;
      if (widget.modelFromTransaction!.image != null) {
        image = File(widget.modelFromTransaction!.image!);
      }
    }
    super.initState();
  }
                  ...
                  children: <Widget>[
                    ...
                    Expanded(
                      child: TextFormField(
                        ...
                        cursorColor: AppColor.ftTextSecondaryColor,
                  ...
                  children: <Widget>[
                    ...
                    Expanded(
                      child: TextFormField(
                        ...
                        decoration: InputDecoration(
                            prefixText: "${currencySymbolUpdate.value} ",
                    ...
                    children: [
                      ElevatedButton(
                        style: ElevatedButton.styleFrom(
                          backgroundColor: AppColor.ftTextLinkColor2,
                        ...
                        child: Text(
                          widget.modelFromTransaction == null
                              ? 'Simpan'
                              : 'Update',
                          style: const TextStyle(
                            color: AppColor.ftTextSecondaryColor,
      ...
      items: [
        PopupMenuItem(
            ...
            child: const Text(
              'Camera',
              style:
                  TextStyle(fontSize: 14, color: AppColor.ftTextSecondaryColor),
            )),
        PopupMenuItem(
  ...
  Future addExpenseTransaction() async {
    ...
    if (widget.modelFromTransaction == null) {
      ...
      final model = TransactionModel(
        id: widget.modelFromTransaction!.id,
        date: DateFormat('yyyy-MM-dd').format(selectedDate),
        amount: parsedAmount ?? 0.0,
        account: getAccountTypeFromString(accountType) ?? AccountType.cash,
        categoryType: selectedCategoryType,
            ...
            TextButton(
              onPressed: () {
                TransactionDB.instance
                    .deleteTransaction(widget.modelFromTransaction!);
                textFeildClear();
